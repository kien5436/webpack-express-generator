...
async function webpackProdMiddleware(req, res, next) {
  ...
  try {
    if (!res.app.get('webpackStats')) {
      ...
      await webpackClose();
      const { assets, errors, warnings } = stats.toJson(statsOptions);

      req.app.set('webpackStats', { assets, errors, warnings });
    }
...
module.exports = 'production' !== NODE_ENV ? [webpackDevMiddleware(compiler, {
  ...
  const { assets, errors, warnings } = res.locals.webpack.devMiddleware.stats.toJson(statsOptions);

  res.app.set('webpackStats', { assets, errors, warnings });
